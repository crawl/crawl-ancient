...
//for (ady = (you.y_pos - 6); (ady < you.y_pos + 7); ady++)
            //{
            // for (adx = (you.x_pos - 6); (adx < you.x_pos + 7); adx++)
            //      {
            if (see_grid(adx, ady) == 1)
            {
                if (igrd[adx][ady] == 501)
...
int animate_a_corpse(char axps, char ayps, int corps_beh, int corps_hit, char class_allowed)
{
    ...
    if (mitm.base_type[igrd[axps][ayps]] != 14)
        return 0;

    if (class_allowed == 1 && mitm.sub_type[igrd[axps][ayps]] != 1)
        return 0;
...
int raise_corpse(int corps, char corx, char cory, int corps_beh, int corps_hit, int actual)
{
    ...
    if (mitm.sub_type[corps] == 0)
        create_monster(MONS_SMALL_ZOMBIE, 0, corps_beh, corx, cory, corps_hit, mitm.pluses[corps]);
    else
        create_monster(MONS_SMALL_SKELETON, 0, corps_beh, corx, cory, corps_hit, mitm.pluses[corps]);

//strcpy(info, monam (250, mitm.pluses [corps], 0, 0));
