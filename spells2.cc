...
void cast_twisted(int power, int corps_beh, int corps_hit)
{
 ...
 finished:                       // now to raise the thing
  ...
  strcpy(info, "The heap of corpses melds into an agglomeration of "
         "writhing flesh!");
  mpr(info);
...
{
  int mon, powtemp, powleft = pow, created = 0;
  int max = pow >>4;
  if (max > 5) max = 5;
  if (max < 1) max = 1;

  while (powleft > 0 && created < max) {
    created ++;
    powtemp = random2(powleft) +1;
    powleft -= powtemp;
    switch(powtemp)
      {
      case 75: case 74:
      ...
      case 20: case 18: case 16: case 14: case 12: case 10:
        ...
        break;
      default:
        if (powtemp % 2) mon = MONS_GIANT_BAT;
        else             mon = MONS_RAT;
        break;
      }
    create_monster(mon, 22, 7, you.x_pos, you.y_pos, MHITNOT, 250);
  }
} // end of (NEW) summon_small_mammals(int pow)
  ...
  for (scount = 0; scount < numsc; scount++)
    {
      switch (random2(14))
        ...
        case 3:
          thing_called = 22;  //jmf: technically not insects but still cool
          break;              // scorpion
        ...
        case 6:
          thing_called = 169; //jmf: blowfly instead of queen bee (was 42)
          break;

        case 8:
          thing_called = 170; //jmf: spiders aren't insects (was 62)
          break;              //jmf: red wasp; was wolf spider
...
void summon_undead(int pow)
{
  ...
  if (you.is_undead == 0 && ! (you.religion == GOD_KIKUBAAQUDGHA
                               && you.piety >= 100
...
void summon_things(int pow)
{
  ...
  //for (scount = 0; scount < numsc; scount ++)
  //jmf: FIXME: are SMALL and LARGE reversed?!
  while (big_things > 0) {
    create_monster(MONS_SMALL_ABOMINATION, 22, 7,
                   you.x_pos, you.y_pos, MHITNOT, 250);
    big_things--;
  }

  while (numsc > 0) {
    create_monster(MONS_LARGE_ABOMINATION, 22, 7,
                   you.x_pos, you.y_pos, MHITNOT, 250);
    numsc--;
  }
