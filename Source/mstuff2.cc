...
// the while portion seems fishy to me for some reason {dlb}
    ...
    while ((grd[monster->x][monster->y] != ogrid && (ogrid != DNGN_DEEP_WATER || grd[monster->x][monster->y] != DNGN_SHALLOW_WATER)) || mgrd[monster->x][monster->y] != NON_MONSTER);
    ...
    if ( launched )
    {
        ...
        if ( ( mitm.special[monster->inv[MSLOT_WEAPON]] % 30 == SPWPN_FLAME
            ...
            || ( mitm.special[hand_used] % 30 == SPMSL_FLAME
                && mitm.special[monster->inv[MSLOT_WEAPON]] % 30 != SPWPN_FROST ))
        {
            baseHit += 2;
            exDamBonus += 6;
            pbolt->flavour = BEAM_FIRE;
        ...
        if ( ( mitm.special[monster->inv[MSLOT_WEAPON]] % 30 == SPWPN_FROST
                && mitm.special[hand_used] % 30 != SPMSL_FLAME )
            || ( mitm.special[hand_used] % 30 == SPMSL_ICE
                && mitm.special[monster->inv[MSLOT_WEAPON]] % 30 != SPWPN_FLAME ))
        {
            baseHit += 2;
            exDamBonus += 6;
            pbolt->flavour = BEAM_COLD;
