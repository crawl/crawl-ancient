        ...
        if ( you.inv_dam[item_wield_2] != SPWPN_NORMAL )
        {
            ...
            switch ( i_dam )
            {
                ...
                case SPWPN_DISTORTION:
                  ...
                  if (effect > 0)
                    miscast_effect(SPTYP_TRANSLOCATION, effect, 90, 100);
                  break;
...
static void throw_it( struct bolt *pbolt, int throw_2 )
{
    ...
    // special: might be throwing generic weapon; use base wep. damage, w/ penalty
    if ( wepClass == OBJ_WEAPONS )
    {
        baseDam = property(wepClass, wepType, PWPN_DAMAGE) - 4;
        if (baseDam < 0) baseDam = 0;
    }
    ...
    if (thrown)
    {
            ...
            (wepClass == OBJ_MISSILES && wepType == MI_STONE))
        {
            ...
            if ( you.inv_dam[throw_2] / 30 == DWPN_ELVEN && player_genus(GENPC_ELVEN) )
                baseHit += 1;
            ...
            exDamBonus = (10 * ( you.skills[SK_THROWING] / 2 + you.strength - 10 )) / 12;
