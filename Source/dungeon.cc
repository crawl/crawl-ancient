...
static int vault_grid( int vx, int vy, int altar_count, FixedVector<char, 7>& acq_item_class, FixedVector<int, 7>& mons_array, char vgrid );
...
int place_monster( unsigned char plus_seventy, int typed, bool is_summoning, int px, int py, char behaviour, int hitting, bool allow_bands, int many_many, FixedVector<int, 2>& passed )
{
...
give_up_on_band:
        ...
        else if ( grid_ok == DNGN_FLOOR )
        {
            ...
            while ( ( grd[menv[bk].x][menv[bk].y] < DNGN_FLOOR || mgrd[menv[bk].x][menv[bk].y] != NON_MONSTER )
                       || ( passed[1] == 0
...
static void special_room( void )
{
    ...
    FixedVector<int, 10> mons_alloc;    // was [20], but last ten values never set or used {dlb}
...
give_armour:
    ...
    if ( menv[bk].inv[MSLOT_WEAPON] != NON_ITEM && launches_things(mitm.sub_type[menv[bk].inv[MSLOT_WEAPON]]) )
    {
        for (bp = 0; bp < MAX_ITEMS - 100; bp++)
          if ( mitm.quantity[bp] == 0 || mitm.base_type[bp] == OBJ_UNASSIGNED )
            break;
...
static void place_spec_shop( unsigned char shop_x, unsigned char shop_y, unsigned char force_s_type )
{
    ...
    env.shop_level[i] = many_many << 1;

    env.shop_type[i] = ( (force_s_type != SHOP_RANDOM) ? force_s_type : random2(NUM_SHOPS) );
    ...
    for (j = 0; j < plojy; j++)
    {
        ...
        orb = items(1, item_in_shop(env.shop_type[i]), OBJ_RANDOM, 0, item_level, 250);
