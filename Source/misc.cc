...
// after the corpse has been butchered ... {dlb}
    if ( monster_descriptor(mcls, MDSC_LEAVES_HIDE) && !one_chance_in(3) )
    {
        for (o = 0; o < MAX_ITEMS; o++)
        {
            if ( o >= 200 + random2(150) )    // should be a way to place this outside loop for efficiency reasons {dlb}
              return;
            ...
            if (mitm.quantity[o] == 0)
            {
                ...
                switch ( mcls )
                {
                  ...
                  case MONS_SWAMP_DRAGON:
                    ...
                    break;
                  default:
                    mitm.sub_type[o] = ARM_ANIMAL_SKIN;    // future implementation {dlb}
                    break;
                }
...
char search_around( void )
{
    ...
    for (srx = you.x_pos - 1; srx < you.x_pos + 2; srx++)
    {
        for (sry = you.y_pos - 1; sry < you.y_pos + 2; sry++)
        {
            ...
            if (grd[srx][sry] == DNGN_SECRET_DOOR && random2(17) <= 1 + you.skills[SK_TRAPS_DOORS])
            ...
            if (grd[srx][sry] == DNGN_UNDISCOVERED_TRAP && random2(17) <= 1 + you.skills[SK_TRAPS_DOORS])
            {
...
void up_stairs( void )
{
    ...
    if ( (stair_find < DNGN_STONE_STAIRS_UP_I || stair_find > DNGN_ROCK_STAIRS_UP )
    ...
    if ( you.your_level == 0 && !yesno("Are you sure you want to leave the Dungeon?") )
    {
...
/*load(stair_taken, moving_level, level_saved, was_a_labyrinth, old_level, want_followers, just_made_new_lev); */
    if (you.where_are_you == BRANCH_VESTIBULE_OF_HELL)
      you.your_level = 27;
    load(stair_taken, moving_level, false, old_level, want_followers, false, old_level_where);
    moving_level = false;
    ...
    for (count_x = 0; count_x < GXM; count_x++)
      ...
      {
          if ( mgrd[count_x][count_y] != NON_MONSTER && ( menv[mgrd[count_x][count_y]].type == -1 || menv[mgrd[count_x][count_y]].x != count_x || menv[mgrd[count_x][count_y]].y != count_y ) )
          {
              mgrd[count_x][count_y] = NON_MONSTER;   /* This is one of the worst things I've ever done */
          }

          if ( grd[count_x][count_y] >= DNGN_BLUE_FOUNTAIN && grd[count_x][count_y] < DNGN_PERMADRY_FOUNTAIN
...
void down_stairs(bool remove_stairs, int old_level)
{
        ...
        if ( num_runes < NUMBER_OF_RUNES_NEEDED )
        {
            switch ( NUMBER_OF_RUNES_NEEDED )
            {
              case 1:
                ...
                break;
              default:
                sprintf(info, "You need at least %d Runes to enter this staircase.", NUMBER_OF_RUNES_NEEDED);
                mpr(info);
            }
            return;
        }
    ...
    load(stair_taken, moving_level, was_a_labyrinth, old_level, want_followers, false, old_where);
...
void new_level( void )
{
    ...
    if ( you.where_are_you >= BRANCH_DIS && you.where_are_you < BRANCH_ORCISH_MINES )  // maybe last part better expresssed as <= PIT {dlb}
      itoa(you.your_level - 26, temp_quant, 10);

/* Remember, must add this to the death_string in ouch */
    if ( you.where_are_you >= BRANCH_ORCISH_MINES && you.where_are_you <= BRANCH_SWAMP )
      itoa(you.your_level - you.branch_stairs[you.where_are_you - 10], temp_quant, 10);
...
static void dart_trap( bool trap_known, int trapped, struct bolt *pbolt )
{
    ...
    if ((20 + (you.your_level << 1)) * random2(200) / 100 >= player_evasion() + random2(you.dex) / 3 - 2 + (you.duration[DUR_REPEL_MISSILES] * 10)
...
void handle_traps( char trt, int i, bool trap_known )
{
    ...
    switch (trt)
    {
      ...
      case TRAP_BLADE:
        ...
        else if ( random2limit(player_evasion(), 40) + (random2(you.dex) / 3) + (trap_known ? 3 : 0) > 8 )
          mpr("A huge blade swings just past you!");
        else
        {
            mpr("A huge blade swings out and slices into you!");
            ouch((you.your_level << 1) + random2avg(29,2) - random2(1 + player_AC()), 0, KILLED_BY_TRAP);
        }
        break;

      case TRAP_ZOT:
      default:
        mpr( (trap_known) ? "You enter the Zot trap." : "Oh no! You have blundered into a Zot trap!" );
        miscast_effect(SPTYP_RANDOM, random2(30) + you.your_level, 75 + random2(100), 3);
        break;
    }
...
void disarm_trap( struct dist disa[1] )
{
    ...
    grd[you.x_pos + disa[0].move_x][you.y_pos + disa[0].move_y] = DNGN_FLOOR;
...
void manage_clouds( void )
{
    ...
    unsigned int dissipate = 0;    // amount which cloud dissipates - must be unsigned! {dlb}
...
// water -> flaming clouds:
        if ( ( env.cloud_type[cc] == CLOUD_FIRE || env.cloud_type[cc] == CLOUD_FIRE_MON )
            && grd[env.cloud_x[cc]][env.cloud_y[cc]] == DNGN_DEEP_WATER )
          {
            dissipate <<= 2;    // that is, multiply by four {dlb}
          }
// lava -> freezing clouds:
        else if ( ( env.cloud_type[cc] == CLOUD_COLD || env.cloud_type[cc] == CLOUD_COLD_MON )
...
// double the amount when slowed - must be applied last(!):
        if ( you.slow )
          dissipate <<= 1;    // that is, multiply by two {dlb}
...
void fall_into_a_pool( bool place, unsigned char grype )
{
    ...
    if ( grype == DNGN_LAVA )
    {
        ...
        {
            strcat(info, "!");    // should boost # of bangs per damage in the future {dlb}
            mpr(info);
            ouch( (10 + random2avg(58,3)) / (player_res_fire() - 100), 0, KILLED_BY_LAVA);
        }
    }
...
// prevents stepping into a thin stream of lava to get to the other side.
    ...
    {
        if ( !place )    // that is, don't display following when fall from levitating
...
// Must start with a consonant!
    strcpy(wc, (coll_div16 ==  0
                 || coll_div16 ==  7) ? "brilliant" :
               (coll_div16 ==  1
                 || coll_div16 ==  8) ? "pale" :
               (coll_div16 ==  2
                 || coll_div16 ==  9) ? "mottled" :
               (coll_div16 ==  3
                 || coll_div16 == 10) ? "shimmering" :
               (coll_div16 ==  4
                 || coll_div16 == 11) ? "bright" :
               (coll_div16 ==  5
                 || coll_div16 == 12) ? "dark" :
               (coll_div16 ==  6
                 || coll_div16 == 13) ? "shining"
                                      : "faint" );
...
bool trap_item( char base_type, char sub_type, char beam_x, char beam_y )
{
    ...
    {
        if ( ( base_type == OBJ_MISSILES    // that is, stackable items present {dlb}
                || base_type == OBJ_FOOD
...
int trap_at_xy( int which_x, int which_y )
{
    ...
    for (int which_trap = 0; which_trap < MAX_TRAPS; which_trap++)
      if ( env.trap_x[which_trap] == which_x && env.trap_y[which_trap] == which_y )
        return which_trap;
