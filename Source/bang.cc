...
{
    char clouty = 0;
    FixedArray < unsigned char, 5, 5 > empty;
    ...
    for (n = 0; n < 5; n++)     // x
    {
        for (q = 0; q < 5; q++) // y
        {
            ...
            if (grd[ pbolt.source_x - 2 + n ]
                   [ pbolt.source_y - 2 + q ] < DNGN_LAST_SOLID_TILE)
            {
                empty[n][q] = 0;
            }
        }
    ...
    for (n = -2; n < 3; n++)    // x
    {
        for (q = -2; q < 3; q++)        // y
        {
            ...
            if (empty[n + 2][q + 2] == 0
                || grd[pbolt.bx][pbolt.by] < DNGN_LAST_SOLID_TILE)
            {
                continue;
            }
        ...
        for (q = -2; q < 3; q++)        // y
        {
            ...
            {                   // ^^^ digging
                ...
                if (you.equip[EQ_BODY_ARMOUR] != -1
                    && (!player_light_armour() && one_chance_in(4)
                        && mass( OBJ_ARMOUR,
                ...
                if (pbolt.flavour == BEAM_FIRE || pbolt.flavour == BEAM_LAVA
            ...
            if (env.cgrid[pbolt.bx][pbolt.by] != EMPTY_CLOUD)
            {
                ...
                {
                    if (see_grid(pbolt.bx, pbolt.by)
                            && !silenced(pbolt.bx, pbolt.by)
                            && !silenced(you.x_pos, you.y_pos))
                    {
                        // what is the range of hearing? {dlb}
                        mpr("You hear a sizzling sound!");
                    }

                    env.cloud_type[clouty] = CLOUD_NONE;
                    env.cgrid[env.cloud_x[clouty]][env.cloud_y[clouty]]
                                                                = EMPTY_CLOUD;
            ...
            {
                place_cloud(CLOUD_COLD, pbolt.bx, pbolt.by,
                                                        2 + random2avg(5, 2));
            }
