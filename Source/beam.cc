...
void beam(struct bolt &pbolt)
{
    ...
    for (n = 1; n < pbolt.range; n++)
    {
        ...
        if (strcmp(pbolt.beam_name, "blast of poison") == 0)
        {
            ...
            {
                place_cloud(CLOUD_POISON, pbolt.bx, pbolt.by,
                            random2(4) + 2);
            }
            else
            {
                place_cloud(CLOUD_POISON_MON, pbolt.bx, pbolt.by,
                            random2(4) + 2);
            }
        ...
        else if (grd[pbolt.bx + pbolt.move_x][pbolt.by + pbolt.move_y]
                                                                    >= MINMOVE)
        {
            pbolt.bx += pbolt.move_x;
            pbolt.by += pbolt.move_y;
        ...
        else
        {
            ...
            if ((pbolt.colour == WHITE
                    && pbolt.flavour != BEAM_COLD
                    && pbolt.hit >= 15)
                || pbolt.flavour == BEAM_NUKE)
            {
                const int targ_x = pbolt.bx + pbolt.move_x;
                const int targ_y = pbolt.by + pbolt.move_y;
                const int targ_grid = grd[ targ_x ][ targ_y ];

                if ((targ_grid == DNGN_ROCK_WALL || targ_grid == DNGN_WAX_WALL)
                     && !(pbolt.bx <= 6 || pbolt.by <= 6
                         || pbolt.bx >= (GXM - 6) || pbolt.by >= (GYM - 6)))
                {
            ...
            {
                const int dir_x = pbolt.bx + pbolt.move_x;
                const int dir_y = pbolt.by + pbolt.move_y;

                if (grd[ pbolt.bx ][ dir_y ] >= MINMOVE
                     && grd[ dir_x ][ pbolt.by ] >= MINMOVE)
                {
                    beam_sign_x *= -1;
                    beam_sign_y *= -1;
                }

                if (grd[ dir_x ][ pbolt.by ] < MINMOVE
                     && grd[ pbolt.bx ][ dir_y ] >= MINMOVE)
                {
                    beam_sign_x *= -1;
                }
...
/*      if (pbolt.bx > you.x_pos - 9 && pbolt.bx < you.x_pos + 9 && pbolt.by > you.y_pos - 9 && pbolt.by < you.y_pos + 9 && (show [pbolt.bx - you.x_pos + 9] [pbolt.by - you.y_pos + 9] != 0 || (pbolt.bx == you.x_pos && pbolt.by == you.y_pos)) && pbolt.type != 0) */
        if (((pbolt.bx > you.x_pos - 9 && pbolt.bx < you.x_pos + 9
                    && pbolt.by > you.y_pos - 9 && pbolt.by < you.y_pos + 9)
        ...
        if (env.cgrid[pbolt.bx][pbolt.by] != EMPTY_CLOUD)     // hit a cloud
        {
            /* polymorph randomly changes clouds in its path */
            // this needs to be fixed to take into account player/monster
            // clouds {dlb}
            if (pbolt.beam_name[0] == '0' && pbolt.colour == DARKGREY)
                env.cloud_type[env.cgrid[pbolt.bx][pbolt.by]] = 1 + random2(8);
...
****************************************************************** */
            ...
            if (pbolt.beam_name[0] != '0')     // ie enchantments always hit
            {
                if (player_light_armour()
                    && pbolt.move_x != 0 && pbolt.move_y != 0 && coinflip())
                {
                ...
                if (you.duration[DUR_DEFLECT_MISSILES])
                    pbolt.hit = random2(pbolt.hit / 3);

                if (pbolt.hit < random2limit(player_evasion(), 40)
                                                + random2(you.dex) / 3 - 2
            ...
            // that is, no "named" beam has been passed - I smell a hack {dlb}
            // hmm... {dlb} is apparently not a mummy -- bwr
            if (pbolt.beam_name[0] == '0')
            {
                pbolt.aim_down = 0;
...
// if ( (pbolt.thing_thrown == KILL_YOU || pbolt.thing_thrown == KILL_YOU_MISSILE) && you_resist_magic(pbolt.ench_power) )
                ...
                if (pbolt.colour != BLUE && pbolt.colour != MAGENTA
                    && pbolt.colour != GREEN
                ...
                case LIGHTMAGENTA:      // pain
                    ...
                    if (YOU_KILL(pbolt.thing_thrown))
                        ouch(1 + random2(pbolt.hit), 0, KILLED_BY_TARGETTING);
                    else
                    {
                        ouch(1 + random2(pbolt.hit), pbolt.beam_source,
                             KILLED_BY_BEAM);
                    }
                    pbolt.wand_id = 1;
                    return;
                ...
                case WHITE:
                    ...
                    {
                        ouch(random2(pbolt.hit), pbolt.beam_source,
                             KILLED_BY_BEAM);
                    }
                    pbolt.wand_id = 1;
                    return;
      ...
      out_of_hit_you:
        if (pbolt.target_x == pbolt.bx
            && pbolt.target_y == pbolt.by
            && pbolt.aim_down == 1
            && ((grd[pbolt.bx][pbolt.by] == DNGN_LAVA
                 && pbolt.flavour == BEAM_COLD)
                || ((grd[pbolt.bx][pbolt.by] == DNGN_DEEP_WATER
                     || grd[pbolt.bx][pbolt.by] == DNGN_SHALLOW_WATER)
                    && pbolt.flavour == BEAM_FIRE)))
        {
            place_cloud(CLOUD_STEAM, pbolt.bx, pbolt.by, 2 + random2(5));
        }

        if (mgrd[pbolt.bx][pbolt.by] != NON_MONSTER
            && (pbolt.colour != BROWN || pbolt.beam_name[0] != '0'))
        {
            ...
            else
            {
                ...
                //       "     = 1 = hits monster before you (possibly after)
                //       "     = 2 = hits monster after but not before
                //       "     = 3 = hits tamed monster(s) but no others
                //       "     = 4 = hits tamed monster(s) + possibly others

                if (monster_habitat(monster->type) != DNGN_FLOOR
                    && monster->number == 1
                    && (pbolt.bx != pbolt.target_x
                        || pbolt.by != pbolt.target_y
                        || pbolt.aim_down != 1))
                {
                    goto check_aimed;
                }

                // isn't this already assumed above ??? {dlb}
                if (pbolt.beam_name[0] != '0') // ie enchantments always hit
                {
                ...
                if (YOU_KILL(pbolt.thing_thrown)
                    && monster->behavior == BEH_ENSLAVED && hurted > 0)
                {
                    monster->behavior = BEH_CHASING_I;
                ...
                if (pbolt.flavour == BEAM_EXPLOSION
                    && strcmp(pbolt.beam_name, "hellfire") != 0)
                {
                    explosion1(pbolt);
                    pbolt.aim_down = 0;
                    return;
          ...
          enchanting:
            if (monster->x == pbolt.bx && monster->y == pbolt.by
                && monster->type != -1 && pbolt.beam_name[0] == '0')
            {
                if (pbolt.colour == 200)
                {
                    if (pbolt.trac_targ != MHITYOU)
                    {
                        ...
                        {
                            if (menv[pbolt.beam_source].behavior ==
                                BEH_ENSLAVED)
                            {
                                pbolt.tracer_mons = 4;
                ...
                if (pbolt.colour == LIGHTGREY) // teleportation
                {
                    if (!check_mons_magres(monster, pbolt.ench_power)
                        && (pbolt.move_x || pbolt.move_y))
                    {
                        goto it_resists;
                    }

                    if (simple_monster_message(monster,
                                                " looks slightly unstable."))
                    {
                        pbolt.wand_id = 1;
                    }
                ...
                if (pbolt.colour == LIGHTGREEN)
                {
                    ...
                    pbolt.aim_down = 0;
                    // need to tie this into messaging outcome {dlb}
                    pbolt.wand_id = 1;
                    return;
                }
                ...
                if (pbolt.colour == YELLOW)
                {
                    ...
                    hurt_monster(monster,
                                 random2(pbolt.hit) + random2(pbolt.hit) +
                                 random2(pbolt.hit));
                ...
                if (!check_mons_magres(monster, pbolt.ench_power)
                    && pbolt.colour != BLUE && pbolt.colour != GREEN
                ...
                if (pbolt.colour == LIGHTMAGENTA)      /* pain/agony */
                {
                    ...
                    {
                        hurt_monster(monster, 1 + random2(pbolt.hit));
                ...
                // I do not think this is right -- should that not be
                // "flavour" and not "colour" {dlb}
                if (pbolt.colour == BEAM_SLEEP
                ...
                // I do not think this is right -- should that not be
                // "flavour" and not "colour" {dlb}
                if (pbolt.colour == BEAM_BACKLIGHT)
                {
                    if (backlight_monsters(monster->x, monster->y,
                                            pbolt.hit, 0))
                    {
                        return;
                    }
...
void missile(struct bolt &pbolt, int throw_2)
{
    ...
    if (pbolt.move_x > 1 || pbolt.move_y > 1
        || pbolt.move_x < -1 || pbolt.move_y < -1)
    {
        if (pbolt.move_x > 0)
            beam_sign_x = 1;
    ...
    for (n = 1; n < pbolt.range; n++)
    {
        ...
        if (grd[pbolt.bx + pbolt.move_x][pbolt.by + pbolt.move_y]
                                                                >= MINMOVE)
        {
            pbolt.bx += pbolt.move_x;
            pbolt.by += pbolt.move_y;
            ...
            if (pbolt.bx == pbolt.target_x && pbolt.by == pbolt.target_y)
            {
                ...
                if (strcmp(pbolt.beam_name, "orb of electricity") == 0
                    || strcmp(pbolt.beam_name, "metal orb") == 0
                    || strcmp(pbolt.beam_name, "great blast of cold") == 0)
                {
                    explosion1(pbolt);
                    pbolt.aim_down = 0;
                    return;
            ...
            {
                place_cloud(CLOUD_PURP_SMOKE, pbolt.bx, pbolt.by,
                            random2(5) + 1);
            }
            ...
            {
                place_cloud(CLOUD_STEAM, pbolt.bx, pbolt.by,
                            random2(5) + 1);
            }

            if (strcmp(pbolt.beam_name, "sticky flame") == 0)
            {
                place_cloud(CLOUD_BLACK_SMOKE, pbolt.bx, pbolt.by,
                            random2(4) + 1);
            }
            ...
            if (strcmp(pbolt.beam_name, "foul vapour") == 0
                && pbolt.bx == pbolt.target_x
                && pbolt.by == pbolt.target_y)
            {
                big_cloud(((pbolt.thing_thrown != KILL_MON)
                                    ? CLOUD_STINK : CLOUD_STINK_MON),
                          pbolt.bx, pbolt.by, 0, 9);
                return;
            }
            ...
            if (pbolt.bx == you.x_pos && pbolt.by == you.y_pos)
            {
                ...
                else
                {
                    ...
                    int hit = random2(pbolt.hit * 5 + 5 * you.shield_blocks);
                    int block = random2(player_shield_class())
                                                + (random2(you.dex) / 5) - 1;
                    ...
                    if (player_shield_class() > 0 && hit <= block)
                    {
                        ...
                        if (strcmp(pbolt.beam_name, "orb of electricity")
                                                                     == 0
                            || strcmp(pbolt.beam_name, "metal orb") == 0
                            || strcmp(pbolt.beam_name, "great blast of cold")
                                                                     == 0)
                        {
                            explosion1(pbolt);
                            pbolt.aim_down = 0;
...
****************************************************************** */
                        ...
                        if (pbolt.flavour == BEAM_LAVA
                            || (pbolt.flavour == BEAM_FIRE
                                && strcmp(pbolt.beam_name, "ball of steam")
                                                                     != 0)
                            || stricmp(pbolt.beam_name, "hellfire") == 0)
                        {
            ...
            // not really the best place for the declaration,
            // but "oh, well" {dlb}
            ...
            if (mgrd[pbolt.bx][pbolt.by] != NON_MONSTER)
            {
                ...
                /* this won't display a "you miss it" if you throw an
                   pbolt.aim_down thing at a lava worm, only if you hit. */
...
/*              if (pbolt.thing_thrown == KILL_YOU_MISSILE || pbolt.thing_thrown == KILL_MON_MISSILE)
   ...
   putch(35);
                        ...
                        if (monster->hit_points < 1)
                        {
                            ...
                            case KILL_YOU_MISSILE:
                                /* You cast a missile spell which doesn't
                                   leave an item. */
                                monster_die(monster, KILL_YOU_MISSILE,
                                            pbolt.beam_source);
                                break;

                            case KILL_MON:      /* mons threw */
                            case KILL_MON_MISSILE:
                                /* mons cast a missile spell which doesn't
                                leave an item. */
                                monster_die(monster, KILL_MON_MISSILE,
                                            pbolt.beam_source);
                                break;
                        ...
                        else
                        {
                            ...
                            if (strstr(pbolt.beam_name, "poison") != NULL
                                && pbolt.flavour != BEAM_POISON
                                && random2(hurted) -
                                random2(monster->armor_class) > 0)
                            {
                                poison_monster( monster,
                                                YOU_KILL(pbolt.thing_thrown) );
                            }
                        }
                    ...
                    else if (YOU_KILL(pbolt.thing_thrown)
                             && mons_near(monster)
                             && (monster_habitat(monster->type) == DNGN_FLOOR
                                 || monster->number == 0))
                                 // No message if monster missile misses
                    {
                        strcpy(info, "The ");
                        strcat(info, pbolt.beam_name);
                        strcat(info, " misses ");
  ...
  landed:
    ...
    {
        if (grd[pbolt.bx][pbolt.by] != DNGN_LAVA
            && grd[pbolt.bx][pbolt.by] != DNGN_DEEP_WATER)
        {
    ...
    if (pbolt.thing_thrown == KILL_MON // monster threw it.
        && throw_2 >= 0
        && (grd[pbolt.bx][pbolt.by] != DNGN_LAVA
            && grd[pbolt.bx][pbolt.by] != DNGN_DEEP_WATER)
        && coinflip())
    {
        ...
        // it is thrown.
        if (mitm.quantity[throw_2] == 0)
        {
        }
        else
        {
            if (igrd[pbolt.bx][pbolt.by] != NON_ITEM)
            {
                if ((mitm.base_type[throw_2] == OBJ_MISSILES
                    ...
                    && mitm.base_type[throw_2] ==
                                mitm.base_type[igrd[pbolt.bx][pbolt.by]]
                    && mitm.sub_type[throw_2] ==
                                mitm.sub_type[igrd[pbolt.bx][pbolt.by]]
                    && mitm.pluses[throw_2] ==
                                mitm.pluses[igrd[pbolt.bx][pbolt.by]]
                    && mitm.pluses2[throw_2] ==
                                mitm.pluses2[igrd[pbolt.bx][pbolt.by]]
                    && mitm.special[throw_2] ==
                                mitm.special[igrd[pbolt.bx][pbolt.by]])
                {
            ...
            // bad bad bad (!!!) using same variable name in the
            // same function -- ick!!! {dlb}
            int o = 0;
