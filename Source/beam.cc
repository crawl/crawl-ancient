...
out_of_hit_you:
        ...
        if ( mgrd[pbolt->bx][pbolt->by] != NON_MONSTER && (pbolt->colour != BROWN || pbolt->beam_name[0] != '0') )
        {
...
int check_mons_resists( struct monsters *monster, struct bolt *pbolt, int hurted )
{
    ...
    case BEAM_FIRE:
        ...
        else if (monster->inv[MSLOT_ARMOUR] != NON_ITEM && mitm.special[monster->inv[MSLOT_ARMOUR]] % 30 == SPARM_FIRE_RESISTANCE)
        {
            simple_monster_message(monster, " resists.");
            hurted /= 3;
        }

        if (mons_res_fire(monster->type) == -1 && (monster->inv[MSLOT_ARMOUR] == NON_ITEM || mitm.special[monster->inv[MSLOT_ARMOUR]] % 30 != SPARM_FIRE_RESISTANCE))
        {
    ...
    case BEAM_COLD:
        ...
        else if (monster->inv[MSLOT_ARMOUR] != NON_ITEM && mitm.special[monster->inv[MSLOT_ARMOUR]] % 30 == SPARM_COLD_RESISTANCE)
        {
            simple_monster_message(monster, " resists.");
            hurted /= 3;
        }

        if (mons_res_cold(monster->type) == -1 && (monster->inv[MSLOT_ARMOUR] == NON_ITEM || mitm.special[monster->inv[MSLOT_ARMOUR]] % 30 != SPARM_COLD_RESISTANCE))
        {
...
void poison_monster( struct monsters *monster, bool fromPlayer )
{
    ...
    if ( monster->inv[MSLOT_ARMOUR] != NON_ITEM && mitm.special[monster->inv[MSLOT_ARMOUR]] % 30 == SPARM_POISON_RESISTANCE )
      return;
...
void place_cloud( unsigned char cl_type, unsigned char ctarget_x,unsigned char ctarget_y, unsigned char cl_range )
{
    ...
    if ( target_cgrid != EMPTY_CLOUD )    // that is, another cloud already there {dlb}
    {
