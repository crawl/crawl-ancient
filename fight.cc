    ...
    if (you.inv_class[you.equip[EQ_WEAPON]] == OBJ_STAVES)
    {
        ...
        if (you.magic_points >= STAFF_COST)
            ...
            case STAFF_COLD:    // FIXME: I don't think I used these right ...
                ...
                if ( defender->inv[MSLOT_ARMOUR] != ING && mitm.special[defender->inv[MSLOT_ARMOUR]] % 30 == SPARM_COLD_RESISTANCE )
                {
                    specdam /= 3;
                }

                if ( mons_res_cold(defender->type) < 0
                    && ( defender->inv[MSLOT_ARMOUR] == ING || mitm.special[defender->inv[MSLOT_ARMOUR]] % 30 != SPARM_COLD_RESISTANCE))
                {
                    specdam += STAFF_DAMAGE(SK_ICE_MAGIC);
                }
            ...
            case STAFF_FIRE:
                ...
                if ( defender->inv[MSLOT_ARMOUR] != ING && mitm.special[defender->inv[MSLOT_ARMOUR]] % 30 == SPARM_FIRE_RESISTANCE )
                {
                    specdam = (random2(damage_done) / 2 + 1) / 3;
                }

                if ( mons_res_fire(defender->type) < 0
                    && (defender->inv[MSLOT_ARMOUR] == ING || mitm.special[defender->inv[MSLOT_ARMOUR]] % 30 != SPARM_FIRE_RESISTANCE))
                {
                    specdam += STAFF_DAMAGE(SK_FIRE_MAGIC);
                }
...
// these are cumulative, of course: {dlb}
        ...
        if ( attacker->type == MONS_TWO_HEADED_OGRE
            && runthru == 1 && attacker->inv[MSLOT_MISSILE] != ING )
          hand_used = 1;
                ...
                || one_chance_in(15)))
        {
            ...
            if ( attacker->inv[hand_used] != ING && mitm.base_type[attacker->inv[hand_used]] == OBJ_WEAPONS && (mitm.sub_type[attacker->inv[hand_used]] < WPN_SLING || mitm.sub_type[attacker->inv[hand_used]] > WPN_CROSSBOW))
            {
        ...
        if ( damage_taken > 0 )
        {
            ...
            if ( attacker->type != MONS_DANCING_WEAPON && mmov_x != ING && mitm.base_type[mmov_x] == OBJ_WEAPONS && !launches_things(mitm.sub_type[mmov_x]) )
            {
                strcat(info, " with ");
...
/* special weapons */
        if ( hit && (attacker->inv[hand_used] != ING || ((attacker->type == MONS_PLAYER_GHOST || attacker->type == MONS_PANDEMONIUM_DEMON) && ghost.values[8] != 0)))
        {
            unsigned char itdam;
...
// cumulative reductions - series of if-conditions is necessary: {dlb}
        ...
        if ( attacker->type == MONS_TWO_HEADED_OGRE
            && runthru == 1
            && attacker->inv[MSLOT_MISSILE] != ING )
          hand_used = 1;
                ...
                && !one_chance_in(20)))
        {
            ...
            if ( attacker->inv[hand_used] != ING && mitm.base_type[attacker->inv[hand_used]] == OBJ_WEAPONS && !launches_things(mitm.sub_type[attacker->inv[hand_used]]) )
            {
        ...
        if ( hit )           //(int) damage_taken > 0)
        {
            ...
            if ( sees )
            {
                ...
                if ( attacker->type != MONS_DANCING_WEAPON && attacker->inv[hand_used] != ING && mitm.base_type[attacker->inv[hand_used]] == OBJ_WEAPONS && !launches_things(mitm.sub_type[attacker->inv[hand_used]]) )
                {
                    strcat(info, " with ");
                    it_name(mmov_x, 3, str_pass);    // was 7
                    strcat(info, str_pass);
...
// special attacks:
        ...
        // deliberate fall-through
            ...
            case MONS_SUN_DEMON:
                ...
                if ( mons_res_fire(defender->type) == -1 || ( defender->inv[MSLOT_ARMOUR] == ING || mitm.special[defender->inv[MSLOT_ARMOUR]] % 30 != SPARM_FIRE_RESISTANCE ) )
        ...
        // intentional fall-through
            ...
            case MONS_ANTAEUS:
                ...
                if (mons_res_cold(defender->type) == -1 && (defender->inv[MSLOT_ARMOUR] == ING || mitm.special[defender->inv[MSLOT_ARMOUR]] % 30 != SPARM_COLD_RESISTANCE))
...
// first, find a suitable spot for the child {dlb}:
    ...
    // loop moves beyond those tiles contiguous to parent {dlb}:
        ...
        // 10-50 for now - must take clouds into account:
            if ( mgrd[parent->x + jex][parent->y + jey] == MNG
...
/* drop weapons & missiles last (ie on top) so others pick up */
    ...
    for (loopy = MSLOT_GOLD; loopy >= MSLOT_WEAPON; loopy--)
    {
        if ( monster->inv[loopy] != ING )
        {
            mitm.link[monster->inv[loopy]] = ING;

            if ( igrd[monster->x][monster->y] == ING )
            {
