...
// int  prop[4][MAX_SUBTYPES][3];

void save_id(char identy[NUM_IDTYPE][MAX_SUBTYPES])
{
    int x = 0, jx = 0;

    for (x = 0; x < NUM_IDTYPE; x++)
    {
        for (jx = 0; jx < MAX_SUBTYPES; jx++)
        {
            identy[x][jx] = id[x][jx];
        }
    }
...
void set_ident_type( int cla, int ty, char setting, bool force )
{
...
static void item_name_2( const item_def &item, char buff[ ITEMNAME_SIZE ],
                         bool terse, bool verbose );
...
const char *it_name( int itn, char des, char buff[ ITEMNAME_SIZE ],
                     bool terse, bool verbose )
{
    item_name( mitm[itn], des, buff, terse, verbose );
    return (buff);
}                               // end it_name()
...
const char *in_name( int inn, char des, char buff[ ITEMNAME_SIZE ],
              bool terse, bool verbose )
{
    item_name( you.inv[inn], des, buff, terse, verbose );

    // for terse listings provide weapon/tool/other in the bracket character
    if (terse && inn == you.equip[EQ_WEAPON] && !player_weapon_wielded())
    {
        if (player_tool_usable())
            buff[1] = ']';
        else
            buff[1] = ':';
    }
                ...
                bool terse, bool verbose )
{
    ...
    item_name_2( item, itm_name, terse, verbose );
    ...
    if (descrip == DESC_INVENTORY_EQUIP && item.x == -1 && item.y == -1)
    {
        ...
        if (item.slot == you.equip[EQ_WEAPON])
        {
            ...
            else if (player_tool_usable())
                strncat( buff, " (usable)", ITEMNAME_SIZE );
            else
                strncat( buff, " (in hand)", ITEMNAME_SIZE );
        }
        else if (item.slot == you.equip[EQ_CLOAK]
                || item.slot == you.equip[EQ_HELMET]
                || item.slot == you.equip[EQ_GLOVES]
                || item.slot == you.equip[EQ_BOOTS]
                || item.slot == you.equip[EQ_SHIELD]
                || item.slot == you.equip[EQ_BODY_ARMOUR])
        {
            strncat( buff, " (worn)", ITEMNAME_SIZE );
        }
        else if (item.slot == you.equip[EQ_LEFT_RING])
        {
            strncat( buff, " (left hand)", ITEMNAME_SIZE );
        }
        else if (item.slot == you.equip[EQ_RIGHT_RING])
        {
            strncat( buff, " (right hand)", ITEMNAME_SIZE );
        }
        else if (item.slot == you.equip[EQ_AMULET])
        {
                         ...
                         bool terse, bool verbose )
{
    ...
    {
    case OBJ_WEAPONS:
        if (!terse && item_ident( item, ISFLAG_KNOW_CURSE ))
        {
        ...
        // term once it's become obsolete.  However, since
        // these do effect price value now, I'm putting up
        // a verbose version for shops. -- bwr
        if (!terse && (!item_ident( item, ISFLAG_KNOW_PLUSES ) || verbose))
        {
        ...
        if (item_ident( item, ISFLAG_KNOW_TYPE ))
        {
            ...
            case SPWPN_VORPAL:
                if (is_range_weapon( item ))
                {
                    strncat(buff, (terse) ? " (velocity)" : " of velocity", ITEMNAME_SIZE );
                    break;
                }
        ...
        if (terse && item_known_cursed( item ))
    ...
    case OBJ_MISSILES:
        ...
        if (brand == SPMSL_POISONED || brand == SPMSL_POISONED_UNUSED)
        ...
        strncat(buff, (item_typ == MI_STONE) ? "stone" :
               ...
               (item_typ == MI_LARGE_ROCK) ? "large rock" : "buggy eggplant",
               ITEMNAME_SIZE);
    ...
    case OBJ_WANDS:
        ...
        {
            const int primary   = (item.special % NUM_WANDS_MAT);
            const int secondary = (item.special / NUM_WANDS_MAT);
    ...
    case OBJ_POTIONS:
        ...
        {
            const int primary   = item.special % NUM_POTIONS_MAT;
            const int secondary = item.special / NUM_POTIONS_MAT;

            strncat(buff,
                   (secondary ==  0) ? "" :
                   (secondary ==  1) ? "bubbling " :
                   (secondary ==  2) ? "lumpy " :
                   (secondary ==  3) ? "fuming " :
                   (secondary ==  4) ? "smoky " :
                   (secondary ==  5) ? "fizzy " :
                   (secondary ==  6) ? "glowing " :
                   (secondary ==  7) ? "sedimented " :
                   (secondary ==  8) ? "metallic " :
                   (secondary ==  9) ? "murky " :
                   (secondary == 10) ? "gluggy " :
                   (secondary == 11) ? "viscous " :
                   (secondary == 12) ? "oily " :
                   (secondary == 13) ? "slimy " :
                   (secondary == 14) ? "emulsified " :
                   (secondary == 15) ? "pungent " :
                   (secondary == 16) ? "effulgent " : "buggy ", ITEMNAME_SIZE);
            ...
            strncat(buff,
                   ...
                   (primary == 11) ? "brown" :
                   (primary == 12) ? "pink" :
                   (primary == 13) ? "white" : "buggy", ITEMNAME_SIZE);
    ...
    case OBJ_FOOD:
        ...
        {
            if (item.plus2 == 1)
                strncat( buff, "partially", ITEMNAME_SIZE );
            else if (item.plus2 == 2)
                strncat( buff, "half", ITEMNAME_SIZE );
            else
                strncat( buff, "mostly", ITEMNAME_SIZE );

            strncat( buff, " eaten ", ITEMNAME_SIZE );
        }
    ...
    case OBJ_SCROLLS:
        ...
        else
        {
            ...
            make_name( item.special, true, buff3 );
            strncat( buff, buff3, ITEMNAME_SIZE );

            // ident property of a ?oRU is that it's been tried
            if (id[IDTYPE_SCROLLS][item_typ] == ID_TRIED_TYPE
                || (id[IDTYPE_SCROLLS][item_typ] == ID_UNKNOWN_TYPE
                    && item_typ == SCR_RANDOM_USELESSNESS
    ...
    case OBJ_JEWELLERY:
        ...
        if (item_ident( item, ISFLAG_KNOW_CURSE ))
        {
            if (item_cursed( item ))
                strncat(buff, "cursed ", ITEMNAME_SIZE );
                    ...
                    && !(pluses && item_ident( item, ISFLAG_KNOW_PLUSES )))
            {
                strncat(buff, "uncursed ", ITEMNAME_SIZE );
            }
        }
        ...
        if (jewellery_is_amulet( item )) // amulets
        {
            if (is_random_artefact( item ))
            {
                strncat(buff, randart_ring_name(item), ITEMNAME_SIZE);
            ...
            const int primary   = item.special % NUM_RINGS_MAT;
            const int secondary = item.special / NUM_RINGS_MAT;

            switch (secondary)
            {
            case 0: default: break;
            case 1: strncat(buff, "square ", ITEMNAME_SIZE ); break;
            case 2: strncat(buff, "thick ", ITEMNAME_SIZE ); break;
            case 3: strncat(buff, "thin ", ITEMNAME_SIZE ); break;
            case 4: strncat(buff, "runed ", ITEMNAME_SIZE ); break;
            case 5: strncat(buff, "blackened ", ITEMNAME_SIZE ); break;
            case 6: strncat(buff, "glowing ", ITEMNAME_SIZE ); break;
            case 7: strncat(buff, "small ", ITEMNAME_SIZE ); break;
            case 8: strncat(buff, "large ", ITEMNAME_SIZE ); break;
            case 9: strncat(buff, "twisted ", ITEMNAME_SIZE ); break;
            case 10: strncat(buff, "tiny ", ITEMNAME_SIZE ); break;
            case 11: strncat(buff, "triangular ", ITEMNAME_SIZE ); break;
            case 12: strncat(buff, "lumpy ", ITEMNAME_SIZE ); break;
            case 13: strncat(buff, "dented ", ITEMNAME_SIZE ); break;
            }

            switch (primary)
            {
            case 0: strncat(buff, "zirconium amulet", ITEMNAME_SIZE ); break;
            case 1: strncat(buff, "sapphire amulet", ITEMNAME_SIZE ); break;
            case 2: strncat(buff, "gold amulet", ITEMNAME_SIZE ); break;
            case 3: strncat(buff, "emerald amulet", ITEMNAME_SIZE ); break;
            case 4: strncat(buff, "garnet amulet", ITEMNAME_SIZE ); break;
            case 5: strncat(buff, "bronze amulet", ITEMNAME_SIZE ); break;
            case 6: strncat(buff, "brass amulet", ITEMNAME_SIZE ); break;
            case 7: strncat(buff, "copper amulet", ITEMNAME_SIZE ); break;
            case 8: strncat(buff, "ruby amulet", ITEMNAME_SIZE ); break;
            case 9: strncat(buff, "ivory amulet", ITEMNAME_SIZE ); break;
            case 10: strncat(buff, "bone amulet", ITEMNAME_SIZE ); break;
            case 11: strncat(buff, "platinum amulet", ITEMNAME_SIZE ); break;
            case 12: strncat(buff, "jade amulet", ITEMNAME_SIZE ); break;
            case 13: strncat(buff, "plastic amulet", ITEMNAME_SIZE ); break;
            case 14: strncat(buff, "orichalcum amulet", ITEMNAME_SIZE ); break;
            case 15: strncat(buff, "electrum amulet", ITEMNAME_SIZE ); break;
            }
        }                       // end of amulets
        else                    // ie is a ring
        {
            ...
            {
            case 0: default: break;
            case 1: strncat(buff, "encrusted ", ITEMNAME_SIZE ); break;
            case 2: strncat(buff, "glowing ", ITEMNAME_SIZE ); break;
            case 3: strncat(buff, "tubular ", ITEMNAME_SIZE ); break;
            case 4: strncat(buff, "runed ", ITEMNAME_SIZE ); break;
            case 5: strncat(buff, "blackened ", ITEMNAME_SIZE ); break;
            case 6: strncat(buff, "scratched ", ITEMNAME_SIZE ); break;
            case 7: strncat(buff, "small ", ITEMNAME_SIZE ); break;
            case 8: strncat(buff, "large ", ITEMNAME_SIZE ); break;
            case 9: strncat(buff, "twisted ", ITEMNAME_SIZE ); break;
            case 10: strncat(buff, "shiny ", ITEMNAME_SIZE ); break;
            case 11: strncat(buff, "notched ", ITEMNAME_SIZE ); break;
            case 12: strncat(buff, "knobbly ", ITEMNAME_SIZE ); break;
            case 13: strncat(buff, "braided ", ITEMNAME_SIZE ); break;
            }

            switch (primary)
            {
            case 0: strncat(buff, "wooden ring", ITEMNAME_SIZE ); break;
            case 1: strncat(buff, "silver ring", ITEMNAME_SIZE ); break;
            case 2: strncat(buff, "gold ring", ITEMNAME_SIZE ); break;
            case 3: strncat(buff, "iron ring", ITEMNAME_SIZE ); break;
            case 4: strncat(buff, "steel ring", ITEMNAME_SIZE ); break;
            case 5: strncat(buff, "bronze ring", ITEMNAME_SIZE ); break;
            case 6: strncat(buff, "brass ring", ITEMNAME_SIZE ); break;
            case 7: strncat(buff, "copper ring", ITEMNAME_SIZE ); break;
            case 8: strncat(buff, "granite ring", ITEMNAME_SIZE ); break;
            case 9: strncat(buff, "ivory ring", ITEMNAME_SIZE ); break;
            case 10: strncat(buff, "bone ring", ITEMNAME_SIZE ); break;
            case 11: strncat(buff, "marble ring", ITEMNAME_SIZE ); break;
            case 12: strncat(buff, "jade ring", ITEMNAME_SIZE ); break;
            case 13: strncat(buff, "glass ring", ITEMNAME_SIZE ); break;
            case 14: strncat(buff, "lapis lazuli ring", ITEMNAME_SIZE ); break;
            case 15: strncat(buff, "obsidian ring", ITEMNAME_SIZE ); break;
            }
        }                       // end of rings
        break;
    ...
    case OBJ_BOOKS:
        if (!item_ident( item, ISFLAG_KNOW_TYPE ))
        {
            const int primary   = item.special / NUM_BOOKS_MAT;
            const int secondary = item.special % NUM_BOOKS_MAT;
            ...
            strncat(buff, (primary == 0) ? "" :
                   ...
                   (primary == 6) ? "dog-eared " :
                   (primary == 7) ? "oblong " :
                   (primary == 8) ? "runed " : "buggily ", ITEMNAME_SIZE);

            strncat(buff, (secondary == 0) ? "" :
                   (secondary == 1) ? "hardcover " :
                   (secondary == 2) ? "leatherbound " :
                   (secondary == 3) ? "metal-bound " :
                   (secondary == 4) ? "papyrus " :
                   (secondary == 5) ? "paperback " : "buggy ", ITEMNAME_SIZE);
        ...
        else
        {
            ...
            strncat(buff, (item_typ == BOOK_MINOR_MAGIC_I
                          ...
                          (item_typ == BOOK_ABJURATION) ? "Abjuration" :
    ...
    case OBJ_STAVES:
        ...
        if (!item_ident( item, ISFLAG_KNOW_TYPE ))
        {
            strncat(buff, (item.special == 0) ? "curved" :
                          ...
                          (item.special == 9) ? "gnarled"
                                              : "buggy", ITEMNAME_SIZE);
            strncat(buff, " ", ITEMNAME_SIZE );
        }

        strncat( buff, "staff", ITEMNAME_SIZE );
...
****************************************************************** */
    ...
    case OBJ_CORPSES:
        if (item_typ == CORPSE_BODY)
        {
            ...
            if (item.plus2 > 0)
                strncat( buff, "partially butchered ", ITEMNAME_SIZE );
        }
    ...
    // Disambiguation
    // XXX: Of course, eventually we might want to replace these
    // with come randomly made books... in which case this simply
    // isn't going to work.
    if (!terse && item_ident( item, ISFLAG_KNOW_TYPE ))
    {
#define name_append(xxx) strncat(buff, (xxx), ITEMNAME_SIZE)
        switch (item.base_type)
        {
        case OBJ_STAVES:
            switch (item_typ)
            ...
            case STAFF_DESTRUCTION_II:
                name_append(" [ice]");
                break;
            case STAFF_DESTRUCTION_III: // XXX: needs a nice short name
                name_append(" [iron,fireball,lightning]");
                break;
            case STAFF_DESTRUCTION_IV:  // XXX: needs a nice short name
        ...
        case OBJ_BOOKS:
            ...
            case BOOK_MINOR_MAGIC_III:
                name_append(" [summoning]");
                break;
            case BOOK_CONJURATIONS_I:
                name_append(" [fire & earth]");
                break;
...
#undef name_append
    ...
    if (it_quant > 1
        ...
        && (item.base_type != OBJ_FOOD || item_typ != FOOD_CHUNK))
    {
        strncat(buff, "s", ITEMNAME_SIZE );
    }
}                               // end item_name_2()
...
static char retvow( int sed )
{
    const char vowels[] = "aeiouaeiouaeiouy  ";
...
static char retlet( int sed )
{
    const char consonants[] = "bcdfghjklmnpqrstvwxzcdfghlmnrstlmnrst";
