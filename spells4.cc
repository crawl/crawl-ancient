...
void place_debris( char x, char y, int debris_type )
{
...
int shatter_monsters(char x, char y, int pow, int garbage)
{
  int damage, monster = mgrd[x][y];
  if (monster == MNG)                   return 0;
...
int shatter_items(char x, char y, int pow, int garbage)
{
...
int shatter_walls(char x, char y, int pow, int garbage)
{
...
//        probably is).
void cast_detect_magic(int pow)
{
  struct dist bmove[1];
  char x, y;
  int monster = 0, item = 0, next; //int max;
  int found[NUM_SPELL_TYPES];
...
int ignite_poison_clouds(char x, char y, int pow, int garbage)
{

    int ctemp;
    bool did_anything = false;

    if (env.cgrid[x][y] != CNG)
    {
        ...
        // is there a better way to find a cloud (e.g. cgrid?)
            for (ctemp = 0; ctemp < CLOUDS; ctemp++)
            {
...
int ignite_poison_monsters(char x, char y, int pow, int garbage)
{
  struct bolt beam[1];
...
int discharge_monsters(char x, char y, int pow, int garbage)
{
...
int distortion_monsters( char x, char y, int pow, int message )
{
...
int passwall( char x, char y, int pow, int garbage )
{
...
int rot_living(char x, char y, int pow, int message)
{
...
int rot_undead(char x, char y, int pow, int garbage)
{
...
int rot_corpses( char x, char y, int pow, int garbage )
{
