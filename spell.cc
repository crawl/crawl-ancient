...
char your_spells(int spc2, int powc, char allow_fail)
{
    ...
    if (allow_fail == 1)
    {
        ...
        if (you.religion != GOD_SIF_MUNA && you.penance[GOD_SIF_MUNA]
                                                    && one_chance_in(40))
        {
            god_speaks( GOD_SIF_MUNA, "Sig Muna laughes at your incompetence." );
            // This will cause failure and increase the miscast effect.
            spfl = - (you.penance[GOD_SIF_MUNA]);
            dec_penance( random2(3) + 1 );
        }
        ...
        if (spfl < spell_fail(spc2))
        {
            ...
            if (you.religion == GOD_SIF_MUNA
                && (!player_under_penance() && you.piety >= 100
    ...
    if (you.religion == GOD_SIF_MUNA
        && you.piety < 200 && random2(12) <= spell_value(spc2))
    {
        gain_piety(1);
    }
