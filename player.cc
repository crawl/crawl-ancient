 ...
 *                                              recovery for Armour.
 *      <2>      5/08/99        LRH             display_char_status correctly
 *                                              handles magic_contamination.
 *      <1>      -/--/--        LRH             Created
 */
...
int player_hunger_rate( void )
{
    ...
    if (you.attribute[ATTR_TRANSFORMATION] == TRAN_AIR)
      return 0; //jmf: hunger isn't fair while you can't eat
...
int player_spell_levels( void )
{
    ...
    int sl = (you.experience_level - 1) + (you.skills[SK_SPELLCASTING] << 1);
...
/* Enchantment skill */
    rm += you.skills[SK_ENCHANTMENTS] << 1;
/* Mutations */
    ...
    if ( you.equip[EQ_BODY_ARMOUR] != -1 )
      {
        ...
        if ( you.inv_type[you.equip[EQ_BODY_ARMOUR]] == ARM_DRAGON_ARMOUR )
          rf += 2;
        else if ( you.inv_type[you.equip[EQ_BODY_ARMOUR]] == ARM_GOLD_DRAGON_ARMOUR )
          rf++;
        else if ( you.inv_type[you.equip[EQ_BODY_ARMOUR]] == ARM_ICE_DRAGON_ARMOUR )
          rf--;
      }
    ...
    if ( you.equip[EQ_BODY_ARMOUR] != -1 )
      {
        ...
        if ( you.inv_type[you.equip[EQ_BODY_ARMOUR]] == ARM_ICE_DRAGON_ARMOUR )
          rc += 2;
        else if ( you.inv_type[you.equip[EQ_BODY_ARMOUR]] == ARM_GOLD_DRAGON_ARMOUR )
          rc++;
        else if ( you.inv_type[you.equip[EQ_BODY_ARMOUR]] == ARM_DRAGON_ARMOUR )
          rc--;
      }
...
// cloaks:
    if ( you.equip[EQ_CLOAK] != -1 && you.inv_dam[you.equip[EQ_CLOAK]] % 30 == SPARM_POISON_RESISTANCE )
      rp++;

// body armour:
    if ( you.equip[EQ_BODY_ARMOUR] != -1 )
      {
        if ( you.inv_dam[you.equip[EQ_BODY_ARMOUR]] % 30 == SPARM_POISON_RESISTANCE )
          rp++;

        if ( you.inv_type[you.equip[EQ_BODY_ARMOUR]] == ARM_GOLD_DRAGON_ARMOUR )
          rp++;
        else if ( you.inv_type[you.equip[EQ_BODY_ARMOUR]] == ARM_SWAMP_DRAGON_ARMOUR )
          rp++;
      }
...
// species:
    if ( you.species == SP_MUMMY || you.species == SP_NAGA || you.species == SP_GHOUL )
      rp++;
    else if ( you.species == SP_GREEN_DRACONIAN && you.experience_level > 6 )
      rp++;
...
int player_AC( void )
{
    ...
    int AC = 0;
    int i;        // loop variable
    int racial_type = ( (player_genus(GENPC_DWARVEN)) ? DARM_DWARVEN :
    ...
    for (i = EQ_CLOAK; i < EQ_LEFT_RING; i++)
    {
        ...
        if ( i == EQ_HELMET && you.inv_plus2[you.equip[i]] >= 2 )    // do we mean to exclude caps here? {dlb}
          continue;
...
// Dwarven armour is universally good
        if ( (int) (you.inv_dam[you.equip[i]] / 30) == DARM_DWARVEN )
          racial_bonus += 2;
            ...
            && (int) (you.inv_dam[you.equip[i]] / 30) == racial_type )
        {
            ...
            // to the correct species, plus the +1 about that anyone
            // gets with dwarven armour.
        ...
        AC += property(OBJ_ARMOUR, you.inv_type[you.equip[i]], PARM_AC) * (15 + you.skills[SK_ARMOUR] + racial_bonus) / 15;
...
//jmf: only give:
        if ( player_genus(GENPC_DRACONIAN) )
        {
            if ( you.experience_level < 8 )
              ...
              AC += 1 + ( you.species == SP_GREY_DRACONIAN ? (you.experience_level - 4) / 2 : (you.experience_level / 4) );
        }
        else
        {
             switch ( you.species )
...
// mutations:
        ...
        AC += you.mutation[MUT_ORANGE_SCALES] + (you.mutation[MUT_ORANGE_SCALES] > 1);
...
int player_evasion( void )
{
    ...
    if ( you.equip[EQ_BODY_ARMOUR] != -1 )
    {
        ...
        ev_change = property(OBJ_ARMOUR, you.inv_type[you.equip[EQ_BODY_ARMOUR]], PARM_EVASION);
        ev_change += you.skills[SK_ARMOUR] / 2;  // lowered from / 3 {bwross}

        if (ev_change > property(OBJ_ARMOUR, you.inv_type[you.equip[EQ_BODY_ARMOUR]], PARM_EVASION) / 3)
          ev_change = property(OBJ_ARMOUR, you.inv_type[you.equip[EQ_BODY_ARMOUR]], PARM_EVASION) / 3;
    ...
    if ( !player_light_armour() )
    {
        ...
        emod += (property(OBJ_ARMOUR, you.inv_type[you.equip[EQ_BODY_ARMOUR]], PARM_EVASION) * 14) / 10;
    }
    ...
    if ( you.mutation[MUT_REPULSION_FIELD] > 0 )
      ev += (you.mutation[MUT_REPULSION_FIELD] << 1) - 1;
...
int burden_change( void )
{
    ...
    for (unsigned char bu = 0; bu < ENDOFPACK; bu++)
    {
        ...
        if ( you.inv_class[bu] == OBJ_CORPSES )
        {
          ...
          you.burden += mass(you.inv_class[bu], you.inv_type[bu]) * you.inv_quantity[bu];
    }
...
// changed the burdened levels to match the change to max_carried
    ...
    if ( you.burden < (max_carried * 5) / 6 )            // (you.burden < max_carried - 1000)
    {
        you.burden_state = BS_UNENCUMBERED;

        if ( old_burdenstate != you.burden_state )
          mpr("You possessions no longer seem quite so burdensome.");     // this message may have to change, just testing {dlb}
    }
    else if ( you.burden < (max_carried * 11) / 12 )     // (you.burden < max_carried - 500)
    {
...
void level_change( void )
{
    ...
    char temp_quant[10];    // why does this need to be ten characters long, exactly? {dlb}

// necessary for the time being, as level_change() is called directly sometimes {dlb}
    you.redraw_experience = 1;

    while ( you.experience_level < 27
           && you.experience > exp_needed(you.experience_level + 2, you.species) )
    {
        ...
        if ( you.experience_level > you.max_level )
        {
            ...
            case SP_ELF:
                ...
                if ( !(you.experience_level % 4) )
                  increase_stats(( coinflip() ? STAT_INTELLIGENCE : STAT_DEXTERITY ), 1, false);
                break;
            ...
            case SP_GREY_ELF:
                ...
                if ( you.experience_level == 5 )
                {
                    mpr("You feel charming!"); //jmf: got Glamour ability
                    you.base_magic_points2++;
                }
#endif
            ...
            case SP_KOBOLD:
                if ( !(you.experience_level % 5) )
                  increase_stats(( coinflip() ? STAT_STRENGTH : STAT_DEXTERITY ), 1, false);
            ...
            case SP_OGRE_MAGE:
                ...
                if ( !(you.experience_level % 5) )
                  increase_stats(( coinflip() ? STAT_INTELLIGENCE : STAT_STRENGTH ), 1, false);
                break;
...
// BEGIN - shouldn't all this be replaced by a call to redraw_screen()??? {dlb}
                    char title[40];
                    ...
                    print_stats();
                    new_level();
// END - shouldn't all this be replaced by a call to redraw_screen()??? {dlb}
                }
...
// END - shouldn't all this be replaced by a call to redraw_screen()??? {dlb}
            ...
            case SP_CENTAUR:
                if ( !(you.experience_level % 4) )
                  increase_stats(( coinflip() ? STAT_DEXTERITY : STAT_STRENGTH ), 1, false);
...
int check_stealth( void )
{
    ...
    if ( you.equip[EQ_CLOAK] != -1 && you.inv_dam[you.equip[EQ_CLOAK]] / 30 == DARM_ELVEN )
      stealth += 20;
...
char *species_name( unsigned char speci )
{
    ...
    if ( player_genus(GENPC_DRACONIAN) && you.experience_level < 7 )
      return "Draconian";     // Causes minor problems with ghosts, but nevermind   // still the case? - 12mar200 {dlb}
...
bool wearing_amulet( char amulet )
{
    ...
    if (amulet == AMU_RESIST_CORROSION || amulet == AMU_CONSERVATION)
      if ( you.equip[EQ_CLOAK] != -1
          && you.inv_dam[you.equip[EQ_CLOAK]] % 30 == SPARM_PRESERVATION)    // this is hackish {dlb}
        return true;
...
void deflate_hp( int new_level, bool floor )
{
    ...
    you.redraw_hit_points = 1;    // must remain outside conditional, given code usage {dlb}
...
void set_mp( int new_amount, bool max_too )
{
    ...
    you.redraw_magic_points = 1;    // must remain outside conditional, given code usage {dlb}
...
char *job_title( int which_job )
{
    ...
    switch ( which_job )
    {
      ...
      case JOB_BERSERKER:
        return "Berserker";
      case JOB_HUNTER:
        return "hunter";          // lc, see "Healer"
      case JOB_CONJURER:
        return "conjurer";        // lc, see "Crusader"
      case JOB_ENCHANTER:
        return "enchanter";       // lc, see "Earth Elementalist"
      ...
      case JOB_CHAOS_KNIGHT:
        return "Knight of Chaos";  // cannot begin "Chaos...", see "Crusader"
      case JOB_TRANSMUTER:
        return "transmuter";      // lc, see "Thief"
      case JOB_HEALER:
        return "Healer";
      case JOB_REAVER:
        return "Reaver";
      case JOB_STALKER:
        return "stalker";         // lc, see "Summoner"
      case JOB_MONK:
        return "Monk";
      case JOB_WARPER:
        return "warper";          // lc, see "Wizard"
      default:
        return "bug hunter";
    }
...
bool player_descriptor( unsigned char which_descriptor )
{
    ...
    {
        case PDSC_UNDEAD:
          if ( you.species == SP_GHOUL || you.species == SP_MUMMY )
            return true;
          break;
